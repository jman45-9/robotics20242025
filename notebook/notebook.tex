...
We have decided to use 2.75in wheels for odometry. We chose this size because it is the smallest size we have. It is beneficial to use the smallest wheels available because the take of the least space. We also must mesure the distance from the tracking center of the robot to the face of each tracking wheels. See the diagram below for how we mesure tracking wheels.
\begin{figure}[h]
 \caption{Mesuring Odom Offsets}
 \label{odom_offset_diagram}
 \begin{tikzpicture}
  \draw rectangle (0,0) rectangle++(10,10);
 \end{tikzpicture}
...
Our job becomes much more difficult if the robot heading changes during the movement. Due to the mathematical complexity of this section most of it is shown in Appendix \ref{app_trighell}. We can now discuss the diffrences between the appendix content and real life application. The most major change is to the $r_C$ and $r_X$ values. We must correct for the distance from the ideal position to the acutal postion of the wheels. Ideally, the odom wheels are position in the exact turn center of the robot, however we often must put them off center. We need to mesure this value and add them to the values of $r_C$ and $r_X$. So the equation for $r_C$ in the code are as follows.
$$ r_C = \frac{C}{\theta} + centerDistance $$
