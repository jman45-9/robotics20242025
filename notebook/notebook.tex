...
This chapter covers the internal structure of the code and how they work together.

\section{Namespaces}
Namespaces are a way to separate code and prevents naming conflicts. 
The significant namespaces for this code are seen in the table, along with their purpose.

\begin{figure}[h]
        \caption{Table of Namespaces}
        \centering
        \begin{tabular}{c|c}
        Namespace & Function \\
        \hline
        highstakes & Primary components for this year \\
        helpers & Secondary components used year to year \\
        pros & PROS Library \\
        lemlib & LemLib Library
    \end{tabular}
\end{figure}

Namespaces can be recognized by their syntax.
All namespaces follow a specific syntax, shown in Figure \ref{CODE::namespacesyntax}.
Namespaces can be declared within another namespace to create sub-namespaces.
However, these are optional.
\begin{figure}[h]
    \caption{Namespace syntax}
    \label{CODE::namespacesyntax}
...
\section{The Robot}
\section{Driver Control Structure}
\section{Autonomous Structure}
